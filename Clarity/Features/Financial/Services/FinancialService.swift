...
class FinancialService {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Clarity", category: "FinancialService")
    ...
    // MARK: - Collections
    private func budgetsCollection() -> CollectionReference {
    ...
    func fetchMonthlyBudget(year: Int, month: Int) async throws -> MonthlyBudget? {
        ...
        let document = try await budgetsCollection().document(documentId).getDocument()
        ...
        logger.info("✅ Fetched budget for \(year)-\(month): €\(budget.estimatedIncome)")
        return budget
    }
    ...
    func saveMonthlyBudget(_ budget: MonthlyBudget) async throws {
        ...
        let documentId = MonthlyBudget.generateDocumentId(userId: userId, year: budget.year, month: budget.month)
        
        try budgetsCollection().document(documentId).setData(from: budgetToSave, merge: true)
    ...
    func updateSavingsAllocated(year: Int, month: Int, amount: Double) async throws {
        ...
        try await budgetsCollection().document(documentId).updateData([
    ...
    func saveGoal(_ goal: Goal) async throws {
        ...
        if let documentId = goal.documentId {
            try goalsCollection().document(documentId).setData(from: goalToSave, merge: true)
        } else {
            try goalsCollection().addDocument(from: goalToSave)
        }
    ...
    func feedPiggyBank(goalId: String, amount: Double, note: String? = nil) async throws {
        ...
        try await goalsCollection().document(goalId).updateData([
