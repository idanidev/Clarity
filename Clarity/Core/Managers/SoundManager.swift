...
final class SoundManager: NSObject {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Clarity", category: "SoundManager")
    ...
    func play(_ effect: SoundEffect) {
        ...
        if let player = players[effect] {
            ...
            // 3. Fallback to System Sound (if file missing)
            // Note: AudioServicesPlayAlertSound overrides silent switch on iPhone.
            // AudioServicesPlaySystemSound does NOT.
            logger.info("⚠️ File not found for \(effect.rawValue). Fallback to SystemID: \(effect.systemSoundID)")
            AudioServicesPlayAlertSound(effect.systemSoundID)
        }
    }
    ...
    private func preloadSounds() {
        for effect in [SoundEffect.startRecording, .endRecording, .success, .error] {
            ...
            do {
                ...
                logger.error("❌ Failed to load sound \(effect.rawValue): \(error.localizedDescription)")
            }
        }
    }
}
